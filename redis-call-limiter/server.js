...
const GLOBAL_RATE_PER_MIN = 10;    // global allowed calls per minute (for testing sir wanted 10)
const MAX_ACTIVE_CALLS = 100;      // global active calls cap
const PENDING_QUEUE = "pending_queue";      // list of job IDs
const PROCESSING_LIST = "processing_list";  // list of job IDs currently processing
const COMPLETED_LIST = "completed_list";    // list of results
const ACTIVE_CALLS = "active_calls";
const TOTAL_MADE = "total_calls_made";
const TOTAL_RESOLVED = "total_calls_resolved";

// helper: minute key string (UTC)
...
// LITERALLY ATOMIC LUA SCRIPT:
// Keys: [ACTIVE_CALLS_KEY, GLOBAL_MIN_KEY, PENDING_QUEUE_KEY, TOTAL_MADE_KEY, PROCESSING_LIST_KEY]
// ARGV: [MAX_ACTIVE, GLOBAL_RATE_LIMIT, SESSION_RATE_LIMIT, minuteSuffix]
const atomicPopLua = `
local active_calls = tonumber(redis.call("GET", KEYS[1]) or "0")
local max_active = tonumber(ARGV[1])
if active_calls >= max_active then
  return nil
end
...
if sessionCount >= sessionLimit then
  -- put job back to end (keep order) and return nil
...
/**
 * Upload endpoint
 * Frontend sends { sessionId: "A"|"B", records: [ {...}, ... ] }
 * We create job entries job:<id> (hash) and push id to pending_queue
 */
...
/**
 * Manual resolve endpoint (not strictly required - processor auto resolves),
 * but provided to allow manual marking of job as resolved if needed.
 * POST { jobId, success: true/false }
 */
app.post("/resolve", async (req, res) => {
  try {
    ...
    const jobExists = await redis.exists(jobHashKey);
    // create a result entry and remove from processing list, decrement active
    const result = {
      jobId,
    ...
    await emitStats();
    // try start more jobs now
    await triggerProcessingOnce();
    return res.json({ message: "resolved" });
  } catch (err) {
    console.error("resolve err", err);
    res.status(500).json({ error: "internal" });
  }
});

/**
 * Stats endpoint used by 3s polling and initial load
 */
app.get("/stats", async (req, res) => {
  try {
    ...
    res.json({
      pending, processing, completed, active, totalMade, totalResolved,
      globalRatePerMin: GLOBAL_RATE_PER_MIN,
      globalRateThisMin: globalRate,
      perSessionLimit: PER_SESSION_RATE,
      sessionRates: { A: rateA, B: rateB }
    });
  } catch (err) {
...
/**
 * tryProcessOnce: invoke Lua to pop one job in an atomic way
...
async function tryProcessOnce() {
  try {
    ...
    // jobHash.payload is stringified; convert
    if (jobHash.payload) {
      try { jobHash.payload = JSON.parse(jobHash.payload); } catch (e) {}
    }
    return jobHash;
  } catch (err) {
    console.error("tryProcessOnce err", err);
    return null;
  }
}

/**
 * When job is started we simulate processing (or call your real function).
 * After done, mark complete: remove from processing list, decrement active, push to completed list, incr resolved
 */
async function handleJob(job) {
  try {
    ...
    // simulate job time
    const durationMs = 1500 + Math.floor(Math.random() * 3000); // 1.5-4.5s
    ...
    await emitStats();
    // after completion, attempt to start more jobs
    await triggerProcessingOnce();
  } catch (err) {
    console.error("handleJob err", err);
  }
}

/**
 * triggerProcessingOnce - try to fetch and start as many jobs as allowed (one-by-one)
...
async function triggerProcessingOnce() {
  ...
  while (true) {
    ...
    started++;
    // start in background
    handleJob(job).catch(err => console.error("handleJob background err", err));
  }
...
/**
 * Background loop - runs every second to pick jobs if allowed.
 * This keeps working even if no explicit triggers.
 */
setInterval(async () => {
  try {
    await triggerProcessingOnce();
